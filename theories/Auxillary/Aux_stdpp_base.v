...
(* FIXME: Move *)
Lemma find_unique_Some `{Finite A} (P : A -> Prop) `{forall a, Decision (P a)}
  (HP : forall a b, P a -> P b -> a = b) a : find P = Some a <-> P a.
Proof.
  split; [apply find_Some|].
  intros Ha.
  specialize (find_is_Some P _ Ha) as (b & Hb & HPb).
  now assert (b = a) as -> by auto.
Qed.

Lemma find_is_Some_iff `{Finite A} (P : A -> Prop) `{forall a, Decision (P a)} :
  is_Some (find P) <-> exists a, P a.
Proof.
  split.
  - intros [a Ha]; exists a; now apply find_Some in Ha.
  - intros [a Ha%(find_is_Some P a)].
    naive_solver.
Qed.


Fixpoint fin_prod {n m} (i : fin n) : fin m -> fin (n * m) :=
  match i with
  | 0%fin => Fin.L (_ * m)
  | FS i => Fin.R m ∘ fin_prod i
  end.

Fixpoint fin_split {n m} : fin (n * m) -> fin n * fin m :=
  match n with
  | 0 => fin_0_inv _
  | S n =>
    fun i => match fin_sum_case i with
      | inl i => (0%fin, i)
      | inr i => prod_map FS id (fin_split i)
      end
  end.

Lemma fin_split_prod {n m} (i : fin n) (j : fin m) :
  fin_split (fin_prod i j) = (i, j).
Proof.
  revert j; induction i; intros j.
  - cbn.
...
Lemma fin_prod_split {n m} (i : fin (n * m)) :
  uncurry fin_prod (fin_split i) = i.
Proof.
  induction n.
  - inv_all_vec_fin.
  - cbn in i |- *.
...
Lemma fin_to_nat_prod {n m} (i : fin n) (j : fin m) :
  fin_to_nat (fin_prod i j) = i * m + j.
Proof.
  induction i; cbn; [rewrite fin_to_nat_L|rewrite fin_to_nat_R]; lia.
Qed.

Lemma fin_to_nat_split_1 {n m} (i : fin (n * m)) :
  fin_to_nat (fin_split i).1 = i / m.
Proof.
  rewrite <- (fin_prod_split i).
  rewrite uncurry_alt, fin_split_prod.
  cbn.
  rewrite fin_to_nat_prod.
  rewrite Nat.div_add_l by (pose (fin_to_nat_lt i); lia).
  rewrite Nat.div_small by apply fin_to_nat_lt.
  lia.
Qed.

Lemma fin_to_nat_split_2 {n m} (i : fin (n * m)) :
  fin_to_nat (fin_split i).2 = i `mod` m.
Proof.
  rewrite <- (fin_prod_split i).
  rewrite uncurry_alt, fin_split_prod.
  cbn.
  ...
  rewrite Nat.Div0.mod_add by (pose (fin_to_nat_lt i); lia).
  rewrite Nat.mod_small by apply fin_to_nat_lt.
  lia.
Qed.
...
Definition fin_inj_inv {n n'} (f : fin n -> fin n') : fin n' -> option (fin n) :=
  fun k' => find (λ k, f k = k').

Global Arguments fin_inj_inv !_ !_ !_ !_ / : assert.

Lemma fin_inj_inv_rinv {n n'} (f : fin n -> fin n') (k' : fin n') k :
  fin_inj_inv f k' = Some k -> f k = k'.
Proof.
  now intros ?%find_Some.
Qed.

Lemma fin_inj_inv_linv {n n'} (f : fin n -> fin n') (Hf : Inj eq eq f) k :
  fin_inj_inv f (f k) = Some k.
Proof.
  apply find_unique_Some, reflexivity.
  now intros ? ? ->%Hf ?%Hf.
Qed.

Lemma fin_inj_inv_is_Some {n n'} (f : fin n -> fin n') k' :
  is_Some (fin_inj_inv f k') <-> exists k, f k = k'.
Proof.
  apply find_is_Some_iff.
Qed.

Definition fin_perm_inv {n} (f : fin n -> fin n) (k : fin n) :=
  default k (fin_inj_inv f k).

Global Arguments fin_perm_inv !_ !_ !_ / : assert.

Lemma fin_perm_inv_spec {n} (f : fin n -> fin n) {Hf : Inj eq eq f} k k' :
  fin_perm_inv f k' = k <-> f k = k'.
Proof.
  split.
  - intros <-.
    assert (Hf' : Surj eq f) by now apply finite_inj_surj.
    destruct (Hf' k') as [k Hk].
    subst k'.
    unfold fin_perm_inv.
    rewrite (fin_inj_inv_linv _ _).
    done.
  - intros <-.
    unfold fin_perm_inv.
    now rewrite fin_inj_inv_linv by done.
Qed.

#[export] Instance fin_perm_inv_rinv {n} (f : fin n -> fin n) (Hf : Inj eq eq f) :
  Cancel eq f (fin_perm_inv f).
Proof.
  intros k.
  now apply (fin_perm_inv_spec _).
Qed.

#[export] Instance fin_perm_inv_linv {n} (f : fin n -> fin n) (Hf : Inj eq eq f) :
  Cancel eq (fin_perm_inv f) f.
Proof.
  intros k.
  now apply (fin_perm_inv_spec _).
Qed.

Lemma fin_inj_inv_to_perm {n} (f : fin n -> fin n) {Hf : Inj eq eq f} k :
  fin_inj_inv f k = Some (fin_perm_inv f k).
Proof.
  unfold fin_perm_inv.
  enough (is_Some (fin_inj_inv f k)) as [? ->] by done.
  apply fin_inj_inv_is_Some.
  now apply finite_inj_surj.
Qed.

#[export] Instance fin_perm_inv_inj {n} (f : fin n -> fin n) (Hf : Inj eq eq f) :
  Inj eq eq (fin_perm_inv f) := cancel_inj.

(* Fixpoint nat_fin_case n (P : nat -> Type) (HPfin : forall (i : fin n), P i)
  (HPlarge : forall k, P (n + i)) : forall k, P k :=
  match n with
  |  *)

Definition fin_inj_to_nat_inj {n n'} (f : fin n -> fin n') (i : nat) : nat :=
  match decide (i < n) with
  | left Hi => f (nat_to_fin Hi)
  | right _ => n' + (i - n)
  end.

Lemma fin_inj_to_nat_inj_compose {n n' n''}
  (g : fin n' -> fin n'') (f : fin n -> fin n') i :
  fin_inj_to_nat_inj g (fin_inj_to_nat_inj f i) =
  fin_inj_to_nat_inj (g ∘ f) i.
Proof.
  unfold fin_inj_to_nat_inj.
  destruct_decide (decide (i < n)) as Hi.
...
#[export] Instance fin_inj_to_nat_inj_inj {n n'} (f : fin n -> fin n') (Hf : Inj eq eq f) :
  Inj eq eq (fin_inj_to_nat_inj f).
Proof.
  intros i j.
  unfold fin_inj_to_nat_inj.
  case_decide as Hi; case_decide as Hj.
  - intros Hij%(inj (fin_to_nat ∘ f))%(f_equal fin_to_nat);
    now rewrite 2 fin_to_nat_to_fin in Hij.
  - pose proof (fin_to_nat_lt (f (nat_to_fin Hi))). lia.
  - pose proof (fin_to_nat_lt (f (nat_to_fin Hj))). lia.
  - lia.
Qed.

Lemma fin_inj_to_nat_inj_fin {n n'} (f : fin n -> fin n') (i : fin n) :
  fin_inj_to_nat_inj f i = f i.
Proof.
  unfold fin_inj_to_nat_inj.
