...
Lemma fold_num_internals' tm es es' :
  length (filter (is_internal (mk_cohg tm es')) es) =
  num_internals (mk_cohg tm es).
...
Lemma mk_node_perm_eq_lengths (tm : TensorMap R A) es es' k :
  es ≡ₚ es' ->
  length (mk_node (mk_cohg tm es) k).1.2 =
  length (mk_node (mk_cohg tm es') k).1.2 /\
  length (mk_node (mk_cohg tm es) k).2 =
  length (mk_node (mk_cohg tm es') k).2.
Proof.
  intros Hes.
  cbn.
  change (is_internal (mk_cohg tm es')) with (is_internal (mk_cohg tm es)).
  change (not_internal (mk_cohg tm es')) with (not_internal (mk_cohg tm es)).
...
Lemma mk_node_perm_eq (tm : TensorMap R A) es es' fint fext k :
  posperm (lengthP (filter (is_internal (mk_cohg tm es)) es)) fint ->
  ppermute fint (filter (is_internal (mk_cohg tm es)) es) =
  filter (is_internal (mk_cohg tm es')) es' ->

  posperm (lengthP (filter (not_internal (mk_cohg tm es')) es')) fext ->
  ppermute fext (filter (not_internal (mk_cohg tm es)) es) =
  filter (not_internal (mk_cohg tm es')) es' ->
  abs_perm
    ((mk_node (mk_cohg tm es) k))
    (relabel_abs (var_map fint fext Datatypes.id) (mk_node (mk_cohg tm es') k)).
Proof.
...
Lemma graph_mabs_perm_eq (tm : gmap nat T) es es' :
  es ≡ₚ es' ->
  graph_mabs (mk_cohg tm es) = graph_mabs (mk_cohg tm es').
Proof.
  intros Hes.
  unfold graph_mabs.
...
(* Lemma graph_semantics_abstracts_is_Some (tm : gmap nat T) es mi mo k
  (mr': Vlist graph_V (replicate (num_internals (mk_cohg tm es)) 0))
     ...
     (get_var graph_V ∅ (graph_ml mi mo) (reverse mr') <$>
      ...
      (mk_internal_var <$>
       filter (λ e : Ty * (Ty * Ty), e.2.1 = k)
         (imap pair (filter (is_internal (mk_cohg tm es)) es))) ++
      (mk_external_var true <$>
       filter (λ e : Ty * (Ty * Ty), e.2.1 = k)
         (imap pair
            (filter (not_internal (mk_cohg tm es)) es))))
   ≫= λ args : list (Vval graph_V),
        graph_mabs (mk_cohg tm es) !! Pos.of_succ_nat k
        ≫= λ fval : Vfunc graph_V,
             Vapplys graph_V fval args) <->
  ((forall i, i ∈ (filter (λ e, e.2 = k /\ ~ is_key (mk_cohg tm es) e.1) es).*1 ->
    (i:>nat) ∈ dom mi) /\
  (forall o, o ∈ (filter (λ e, e.1 = k /\ ~ is_key (mk_cohg tm es) e.2) es).*2 ->
    (o:>nat) ∈ dom mo)) /\ True.
Proof.
  intros Hkdom.
  unfold graph_mabs.
  ...
  - rewrite not_elem_of_list_fmap.
    ...
    + change (Forall _ ?l) with
      ...
      * intros (i, j).
        ...
        intros -> _.
        enough (is_key (mk_cohg tm es) k) by tauto.
        apply elem_of_dom.
...
Lemma num_internals_perm_eq tm es es' : es ≡ₚ es' ->
  @num_internals T (mk_cohg tm es) = num_internals (mk_cohg tm es').
Proof.
...
Lemma graph_semantics_perm_eq_aux (tm : gmap nat T) es es' :
  ...
  es ≡ₚ es' ->
  forall mi mo,
  graph_map_semantics (mk_cohg tm es) mi mo ==
  graph_map_semantics (mk_cohg tm es') mi mo.
Proof.
...
Lemma graph_semantics_perm_eq_aux (tm : TensorMap R A) es es' :
  es ≡ₚ es' ->
  forall mi mo,
  graph_tensorlist_semantics (mk_cohg tm es) =tl=
  graph_tensorlist_semantics (mk_cohg tm es').
Proof.
  intros Hes.
  eexists; cbn -[reverse].
  rewrite !fmap_const.
  remember (num_internals (mk_cohg tm es)) as x eqn:Hx.
  pose proof Hx as Hx'.
  cbn in Hx'.
  rewrite <- Hx', Hx.
  ...
  replace (N.succ_pos _) with (Pos.of_succ_nat (num_internals (mk_cohg tm es)))
    by lia.
  apply (filter_Permutation (is_internal (mk_cohg tm es))) in Hes as Hesint.
