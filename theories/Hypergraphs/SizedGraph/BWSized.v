From TensorRocq Require Import sigT2_relation BW Props SizedProps PropsGraphs 
  SizedGraph.Definitions SizedGraph.ToUnsized.
...
Lemma graph_rel_cast_l {T} (R : forall n m, relation (CospanHyperGraph T n m))
  {n m n' m'} (cohg : CospanHyperGraph T n m) (cohg' : CospanHyperGraph T n' m')
  Hn Hm :
  R n' m' (cast_graph Hn Hm cohg) cohg' <-> R n m
    cohg (cast_graph (eq_sym Hn) (eq_sym Hm) cohg').
Proof.
...
Lemma bw_sized_graph_to_graph_scohg_syntactic_eq {N T} 
  `{Equiv T, Equivalence T equiv} f 
  {n m} (bcohg bcohg' : BWSizedCospanHyperGraph N T n m) : 
  bcohg ≡ₛ bcohg' ->
  (bw_sized_graph_to_graph f bcohg ≡ₛ bw_sized_graph_to_graph f bcohg')%cohg.
Proof.
  intros Heq.
  unfold bw_sized_graph_to_graph.
  apply (sized_graph_to_graph_cohg_syntactic_eq f) in Heq.
  apply graph_rel_cast_l.
  apply sigT2_relation_alt in Heq as (Heq & Hiso).
  cbn [projT2 projT1 graph_to_pair_bundled] in Hiso.
  rewrite Hiso.
  rewrite eq_rect_r_to_cast_graph.
  rewrite cast_graph_cast_graph.
  f_equiv; apply proof_irrel.
...
Definition bw_sized_graph_lunit_r {N T} {n m} (scohg : BWSizedCospanHyperGraph N T n (0 + m)) :
  BWSizedCospanHyperGraph N T n m := {|
...
Lemma bw_sized_graph_to_graph_lunit_r {N T} 
  `{Equiv T, Equivalence T equiv} f 
  {n m} (bcohg : BWSizedCospanHyperGraph N T n (0 + m)) : 
  (bw_sized_graph_to_graph f (bw_sized_graph_lunit_r bcohg) ≡ₛ bw_sized_graph_to_graph f bcohg)%cohg.
Proof.
  unfold bw_sized_graph_to_graph.
  f_equiv; [apply proof_irrel..|done].
Qed.
