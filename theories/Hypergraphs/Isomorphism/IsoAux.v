...
Definition gPmap (K A : Type) :=
  Pmap A.

Global Typeclasses Opaque gPmap.

Global Arguments gPmap _ _ : simpl never.
...
#[export] Instance gPmap_fmap : FMap gPmap := Pmap_fmap.

#[export] Instance gPmap_lookup A : Lookup K A (gPmap A) :=
  fun k m => (m :> Pmap A) !! encode k.

#[export] Instance gPmap_empty {A} : Empty (gPmap A) := Pmap_empty.

#[export] Instance gPmap_partial_alter A : PartialAlter K A (gPmap A) :=
  fun f k m => partial_alter (M:=Pmap A) f (encode k) m.

#[export] Instance gPmap_omap : OMap gPmap := Pmap_omap.

#[export] Instance gPmap_merge : Merge gPmap := Pmap_merge.

#[export] Instance gPmap_fold {A} : MapFold K A (gPmap A) :=
  fun B f b m =>
  Pmap_fold B (fun p a acc =>
    from_option (fun k => f k a acc) acc (decode p)) b m.
...
#[export] Instance Psurj_empty : Empty Psurj :=
  mk_Psurj ∅ ∅ map_preimage_empty.

#[export] Instance Psurj_insert : Insert positive positive Psurj :=
  fun k v m =>
  mk_Psurj (<[k := v]> m.(Psurj_map)) _
    (map_preimage_insert _ _ k v (Psurj_preimage m)).

Definition Psurj_insert_fresh (m : Psurj) (k v : positive) :
  m.(Psurj_map) !! k = None -> 
  Psurj.
refine (
  fun Hmk => 
  mk_Psurj (<[k := v]> m.(Psurj_map))
    (partial_alter (λ X, Some $ from_option ({[k]} ∪.) {[k]} X) v m.(Psurj_invmap))
    _).
abstract (pose proof (map_preimage_insert _ _ k v (Psurj_preimage m)) as H;
      rewrite Hmk in H;
      exact H).
Defined.
...
Definition Psurj_inj_insert k v m : option Psurj :=
  match m.(Psurj_map) !! k with
  | Some v' => if decide (v' = v) then Some m else None
  | None => 
    match m.(Psurj_invmap) !! v with
    | None => Some (<[k := v]> m)
    | Some _ => None
    end
  end.
...
Record WPsurj := mk_WPsurj {
  WPsurj_map : Pmap positive;
  WPsurj_invmap : Pmap Pset
}.

Definition Psurj_to_weak (p : Psurj) : WPsurj := mk_WPsurj p.(Psurj_map) p.(Psurj_invmap).

#[export] Instance WPsurj_empty : Empty WPsurj := mk_WPsurj ∅ ∅.

#[export] Instance WPsurj_insert : Insert positive positive WPsurj :=
  fun k v m =>
  mk_WPsurj (<[k := v]> m.(WPsurj_map)) (
    let '(mk_WPsurj m mpre) := m in 
    match m !! k with
    | None =>
      partial_alter (λ X, Some $ from_option ({[k]} ∪.) {[k]} X) v mpre
    | Some v' =>
...
(* FIXME: All wrong... 
Definition WPsurj_inj_insert k v (m : WPsurj) : option WPsurj :=
  let '(mk_WPsurj m mpre) := m in 
  match m !! v with
  | None => Some $ mk_WPsurj (<[k := v]> m) (<[v := {[k]}]> mpre)
  | Some _ => None
  end. *)