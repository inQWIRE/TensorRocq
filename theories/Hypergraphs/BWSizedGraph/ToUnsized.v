...
Definition bvec_enlarge_Pmap {A B} {nf : A -> btree B}
  ...
  Pmap B :=
  map_bind (λ k a,
    bvec_to_map (bvimap (λ i v, (encode (k, i), v)) (f k a))
  ) m.
...
Lemma bvec_enlarge_Pmap_disjointness {A B} {nf : A -> btree B}
  ...
  forall k k' a a',
  k <> k' ->
  (bvec_to_map (bvimap (λ i v, (encode (k, i), v)) (f k a)) :> Pmap B) ##ₘ
   bvec_to_map (bvimap (λ i v, (encode (k', i), v)) (f k' a')).
Proof.
  intros k k' a a' Hk.
  rewrite map_disjoint_dom, 2 dom_bvec_to_map.

  rewrite 2 bvmap_bvimap.
  cbn.
  unfold compose.
  cbn.
  ...
  now intros (i & <- & _)%elem_of_bvmap (j & [= -> ->]%encode_inj & _)%elem_of_bvmap.
Qed.

Lemma bvec_enlarge_Pmap_dom_eq {A B} {nf : A -> btree B}
  (f : positive -> forall (a : A), bvec B (nf a)) (m : Pmap A) :
  forall k a a' k2,
  nf a = nf a' ->
  is_Some ((bvec_to_map (bvimap (λ i v, (encode (k, i), v)) (f k a)) :> Pmap B) !! k2)
  ↔ is_Some ((bvec_to_map (bvimap (λ i v, (encode (k, i), v)) (f k a')) :> Pmap B) !! k2).
Proof.
  intros k a a' k2 Hnf.
  rewrite <- 2 elem_of_dom.
  rewrite 2 dom_bvec_to_map.
  rewrite 2 bvmap_bvimap.
  cbn.
  rewrite 2 bvimap_bvseq_1.
  now rewrite Hnf.
...
Lemma lookup_bvec_enlarge_Pmap_Some {A B} {nf : A -> btree B}
  ...
  bvec_enlarge_Pmap f m !! k = Some b <->
  exists (i : positive) (j : positive) a, k = encode (i, j) /\
  m !! i = Some a /\ f i a !! j = Some b.
Proof.
  ...
  setoid_rewrite <- elem_of_bvec_to_map. 2:{
    rewrite bvmap_bvimap.
    cbn.
    rewrite bvimap_bvseq_1.
    rewrite bvec_to_vec_bvmap, vec_to_list_map.
    apply (NoDup_fmap _).
    apply NoDup_bvseq.
  }
  setoid_rewrite elem_of_lookup_bvimap.
  naive_solver.
Qed.
...
Lemma lookup_bvec_enlarge_Pmap_not_encode {A B} {nf : A -> btree B}
  ...
  bvec_enlarge_Pmap f m !! k = None.
Proof.
  intros Hk.
  rewrite eq_None_not_Some.
  unfold is_Some.
  intros (b & Hb%lookup_bvec_enlarge_Pmap_Some).
  naive_solver.
Qed.
...
Lemma lookup_bvec_enlarge_Pmap_encode {A B} {nf : A -> btree B}
  ...
  bvec_enlarge_Pmap f m !! (encode (i, j)) =
  m !! i ≫= λ a, f i a !! j.
Proof.
  apply option_eq; intros b.
  rewrite bind_Some.
...
Lemma bvec_enlarge_Pmap_insert {A B} {nf : A -> btree B}
  (f : positive -> forall (a : A), bvec B (nf a))
  k a m : m !! k = None ->
  bvec_enlarge_Pmap f (<[k := a]> m) =
  bvec_to_map (bvimap (λ i v, (encode (k, i), v)) (f k a))
  ∪ bvec_enlarge_Pmap f m.
Proof.
  intros Hmk.
  unfold bvec_enlarge_Pmap.
  rewrite map_bind_insert; [done| |done].
...
Lemma bvec_enlarge_Pmap_disj_union {A B} {nf : A -> btree B}
  (f : positive -> forall (a : A), bvec B (nf a))
  m m' : m ##ₘ m' ->
  bvec_enlarge_Pmap f (m ∪ m') =
  bvec_enlarge_Pmap f m ∪ bvec_enlarge_Pmap f m'.
Proof.
  apply map_bind_disj_union.
  apply bvec_enlarge_Pmap_disjointness.
Qed.
...
Lemma bvec_enlarge_Pmap_disjoint {A B} {nf : A -> btree B}
  (f : positive -> forall (a : A), bvec B (nf a))
  m m' : m ##ₘ m' ->
  bvec_enlarge_Pmap f m ##ₘ bvec_enlarge_Pmap f m'.
Proof.
  intros Hdisj.
  apply map_disjoint_alt.
  intros k.
  destruct (is_encode_dec (positive * positive) k) as [[[i j] <-]|Hk].
  - rewrite 2 lookup_bvec_enlarge_Pmap_encode.
    rewrite map_disjoint_alt in Hdisj.
    specialize (Hdisj i) as [-> | ->]; auto.
  - left.
    now rewrite lookup_bvec_enlarge_Pmap_not_encode by done.
Qed.
...
Definition map_vec_sized_graph {N M} {T n m} {nf : N -> btree M}
  (idxs : forall (k : N) (p : positive), bvec positive (nf k))
  (f : forall k, bvec M (nf k)) (scohg : SizedCospanHyperGraph N T n m) :
  SizedCospanHyperGraph M T (n ≫= nf) (m ≫= nf) :=
    mk_scohg
      (bvbind (λ n a, bvmap (λ k, encode (a, k)) (idxs n a)) scohg.(sized_inputs))
      (bvbind (λ n a, bvmap (λ k, encode (a, k)) (idxs n a)) scohg.(sized_outputs))
      (enlarge_hypergraph (λ p, default [] (
          n ← scohg.(sized_map) !! p;
          Some (vec_to_list $ bvec_to_vec $ bvmap (λ k, encode (p, k)) (idxs n p))
      )) scohg.(sized_hedges))
    (bvec_enlarge_Pmap (λ k a, f a) scohg.(sized_map)).
...
Lemma eval_sized_graph_to_map_vec_sized_graph {N T n m} `{Inhabited A} (nf : N -> nat)
  ...
  (scohg_to_cohg (map_vec_sized_graph (nf:=fun n => nat_to_btree (A:=A) (nf n))
    (λ k p, vec_to_nat_bvec (f k p)) (λ _, bvec_of_fun (λ _, inhabitant) _) scohg)).
Proof.
  apply cohg_ext.
  - cbn.
...
Lemma bvec_enlarge_Pmap_kmap {A B} {nf : A -> btree B}
  ...
  bvec_enlarge_Pmap f (kmap g m) =
  kmap (encode_map (prod_map g (@id positive))) (bvec_enlarge_Pmap (f ∘ g) m).
Proof.
  apply map_eq.
  intros i.
  apply option_eq; intros k.
  rewrite (lookup_kmap_Some _).
  setoid_rewrite lookup_bvec_enlarge_Pmap_Some.
  setoid_rewrite (lookup_kmap_Some g).
  split.
  - intros (_ & j & a & -> & (i' & -> & Hi) & Haj).
    cbn.
    exists (encode (i', j)).
    rewrite encode_map_encode.
    split; [done|].
    naive_solver.
  - intros (_ & -> & i' & j & a & -> & Hi' & Haj).
    rewrite encode_map_encode.
    naive_solver.
Qed.
...
Lemma map_vec_sized_graph_relabel_sized_graph {N M T n m}
  {nf : N -> btree M} (idxs : forall k, positive -> bvec positive (nf k))
  ...
  relabel_sized_graph
    ...
    (map_vec_sized_graph (λ k a, idxs k (g a)) f scohg).
Proof.
  apply scohg_ext.
  - cbn.
  ...
  - cbn.
    rewrite (bvec_enlarge_Pmap_kmap _).
    done.
Qed.
...
Lemma map_vec_bvec_to_set_bvbind_Forallbv_correct
  ...
  bvec_to_set (bvbind (λ (n0 : N) (a : positive),
      bvmap (λ k : positive, encode (a, k)) (idxs n0 a)) v) =@{Pset}
...
Lemma map_vec_sized_graph_norm_sized_verts {N M} {T}
  ...
  {n m} (scohg : SizedCospanHyperGraph N T n m) :
  well_sized scohg ->
  map_vec_sized_graph (λ _ _, bvseq xH _) f (norm_sized_verts scohg) =
  norm_sized_verts (map_vec_sized_graph (λ _ _, bvseq xH _) f scohg).
Proof.
  intros Hscohg.
  ...
  - cbn.
    ...
    + intros (i' & j & k & -> & [Hv Hi'] & Hfj).
      ...
      rewrite Hv.
      cbn.
      rewrite elem_of_list_to_set, elem_of_bvec_to_vec.
      rewrite elem_of_bvmap.
      exists j.
      split; [done|].
      rewrite elem_of_bvseq_lookup_aux.
      now apply mk_is_Some, lookup_bvec_is_Some in Hfj.
    + intros ((i' & j & a & -> & Hi'a & Hfaj) & (i'' & Hi'' & Hi'j)%elem_of_set_bind).
      destruct (_ !! i'') as [a'|] eqn:Ha'; [|easy].
      cbn in Hi'j.
      rewrite elem_of_list_to_set, elem_of_bvec_to_vec, elem_of_bvmap in Hi'j.
      destruct Hi'j as (j' & [= -> ->]%encode_inj & Hj).
      naive_solver.
Qed.
...
(* FIXME: Move *)
Lemma vec_to_nat_bvec_to_vec {N} `{HN : Inhabited N}
  {A n} (v : bvec A (nat_to_btree (A:=N) n)) H :
  vec_to_nat_bvec (Vector.cast (bvec_to_vec v) H) = v.
Proof.
  revert H.
  induction n.
  - inv_all_bvec; done.
...
Lemma map_vec_sized_graph_ext {N M T} {n m : btree N}
  {nf : N -> btree M} (idxs idxs' : forall k, positive -> bvec positive (nf k))
  (f f' : forall k, bvec M (nf k)) :
  (forall k p, idxs k p = idxs' k p) ->
  (forall k, f k = f' k) ->
  forall (scohg : SizedCospanHyperGraph N T n m),
  map_vec_sized_graph idxs f scohg = map_vec_sized_graph idxs' f' scohg.
Proof.
  intros Hidxs Hf scohg.
  apply scohg_ext; cbn.
  - apply bvbind_ext.
    intros.
    now rewrite Hidxs.
...
Lemma eval_sized_graph_struct_sized_isomorphic {N} {T}
  ...
  well_sized scohg ->
  scohg ≡ᵢ scohg' ->
  (eval_sized_graph nf (λ k _, Vector.cast (bvec_to_vec (bvseq 1 _)) (bsize_nat_to_btree (A:=unit) _)) scohg ≡ᵢ
    eval_sized_graph nf (λ k _, Vector.cast (bvec_to_vec (bvseq 1 _)) (bsize_nat_to_btree (A:=unit) _)) scohg')%cohg.
Proof.
  intros HWF Heq.
  pose proof HWF as HWF'.
  rewrite Heq in HWF'.
  revert Heq.
  unfold struct_sized_isomorphic.
  intros Heq%(eval_sized_graph_sized_isomorphic (nf:=nf) (λ k, Vector.cast (bvec_to_vec (bvseq 1 _)) (bsize_nat_to_btree (A:=unit) _))).
  revert Heq.
  rewrite 4 (eval_sized_graph_to_map_vec_sized_graph (A:=unit)).
  do 2 destruct (size_bind_nat_to_btree).
  rewrite 4 cast_graph_id.
  erewrite map_vec_sized_graph_ext; [|intros; rewrite vec_to_nat_bvec_to_vec; reflexivity|intros; reflexivity].
  rewrite map_vec_sized_graph_norm_sized_verts by done.
  erewrite (map_vec_sized_graph_ext (λ _ _, vec_to_nat_bvec _)); [|intros; rewrite vec_to_nat_bvec_to_vec; reflexivity|intros; reflexivity].
  rewrite map_vec_sized_graph_norm_sized_verts by done.
  rewrite 2 scohg_to_cohg_norm_sized_verts.
  intros Heq.
  erewrite map_vec_sized_graph_ext; [|intros; rewrite vec_to_nat_bvec_to_vec; reflexivity|intros; reflexivity].
  erewrite (map_vec_sized_graph_ext (λ _ _, vec_to_nat_bvec _)); [|intros; rewrite vec_to_nat_bvec_to_vec; reflexivity|intros; reflexivity].
  exact Heq.
...
(* TODO: Show isomorphism of eval_sized_graph with different (nodup) vectors.
  Use this to prove the following: *)

(* FIXME: Move *)
Lemma bveval_bvmap {N A B C} (f : N -> nat) (fv : forall n, B -> vec C (f n))
  (g : A -> B) {n : btree N} (v : bvec A n) :
  bveval f fv (bvmap g v) =
  bveval f (λ k a, fv k (g a)) v.
Proof.
  induction v; inv_all_bvec; cbn; f_equal; done.
Qed.

Lemma vmap_bveval {N A B C} (f : N -> nat) (fv : forall n, A -> vec B (f n))
  (g : B -> C) {n : btree N} (v : bvec A n) :
  vmap g (bveval f fv v) =
  bveval f (λ k a, vmap g (fv k a)) v.
Proof.
  induction v; inv_all_bvec; cbn; f_equal; rewrite Vector.map_append; f_equal; done.
Qed.

Lemma bveval_ext {N A B} (f : N -> nat) (fv fv' : forall n, A -> vec B (f n))
  {n : btree N} (v : bvec A n) :
  (forall i k a, n !! i = Some k -> v !! i = Some a -> fv k a = fv' k a) ->
  bveval f fv v = bveval f fv' v.
Proof.
  intros Hfv.
  induction v; [done|..].
  - cbn.
  ...
  - cbn.
    f_equal.
    + apply IHv1.
      intros i; apply (Hfv i~0).
    + apply IHv2.
      intros i; apply (Hfv i~1).
Qed.

Lemma eval_sized_graph_relabel_vecs {N T} {n m : btree N}
  {nf : N -> nat} (idxs : forall k, positive -> vec positive (nf k))
  (f : positive -> positive) (scohg : SizedCospanHyperGraph N T n m) :
  eval_sized_graph nf (λ k p, vmap f (idxs k p)) scohg =
  relabel_graph (encode_map (prod_map (@id positive) f)) (eval_sized_graph nf idxs scohg).
Proof.
  apply cohg_ext.
  - cbn.
    rewrite relabel_hg_enlarge_hypergraph.
...
Qed.
(* FIXME: Move *)
Lemma elem_of_bvec_lookup_2 {N A} {n : btree N} (v : bvec A n) i (a : A) :
  v !! i = Some a -> a ∈ v.
Proof.
  now rewrite elem_of_bvec_lookup; eauto.
Qed.

Lemma well_sized_inputs_outputs {N T} {n m : btree N}
  {scohg : SizedCospanHyperGraph N T n m}
  (HWF : well_sized scohg) i j k :
    bnode n m !! i = Some k -> bvnode scohg.(sized_inputs) scohg.(sized_outputs) !! i = Some j ->
    scohg.(sized_map) !! j = Some k.
Proof.
  revert i j k.
  revert HWF.
  unfold well_sized.
  remember (bvnode _ _) as v eqn:Hv.
  clear Hv.
  remember (bnode n m) as n' eqn:Hn'.
  destruct scohg as [ins outs hg m'].
  cbn.
  clear Hn' n m ins outs hg.
  rename n' into n, m' into m.
  induction v.
  - easy.
  - intros Ha%Forallbv_leaf i j k Hk [-> [= <-]]%lookup_bvleaf_Some.
    cbn in Hk.
    revert Hk.
    now intros [= <-].
  - rewrite Forallbv_node.
    intros [Hv1 Hv2].
    intros [i|i|]; cbn; [now apply IHv2|now apply IHv1|easy].
Qed.

Lemma well_sized_inputs {N T} {n m : btree N}
  {scohg : SizedCospanHyperGraph N T n m}
  (HWF : well_sized scohg) i j k :
    n !! i = Some k -> scohg.(sized_inputs) !! i = Some j ->
    scohg.(sized_map) !! j = Some k.
Proof.
  apply (well_sized_inputs_outputs HWF (i~0)).
Qed.

Lemma well_sized_outputs {N T} {n m : btree N}
  {scohg : SizedCospanHyperGraph N T n m}
  (HWF : well_sized scohg) i j k :
    m !! i = Some k -> scohg.(sized_outputs) !! i = Some j ->
    scohg.(sized_map) !! j = Some k.
Proof.
  apply (well_sized_inputs_outputs HWF (i~1)).
Qed.


Lemma eval_sized_graph_relabel_vecs_gen {N T} {n m : btree N}
  {nf : N -> nat} (idxs : forall k, positive -> vec positive (nf k))
  (f : N -> positive -> positive) (scohg : SizedCospanHyperGraph N T n m) :
  well_sized scohg ->
  eval_sized_graph nf (λ k p, vmap (f k) (idxs k p)) scohg =
  relabel_graph (encode_map
    (λ k_idx, (k_idx.1, default k_idx.2
      (n ← scohg.(sized_map) !! k_idx.1; Some (f n k_idx.2)))))
    (eval_sized_graph nf idxs scohg).
Proof.
  intros HWF.
  apply cohg_ext.
  - cbn.
    ...
    intros i Hi.
    cbn.
    destruct (_ !! i) as [k|] eqn:Hmi; [|done].
    cbn.
    rewrite <- vec_to_list_map.
    rewrite 2 Vector.map_map.
    f_equal.
  ...
  - cbn.
    rewrite vmap_bveval.
    apply bveval_ext.
    intros i k j Hni Hvi.
    rewrite 2 Vector.map_map.
    apply Vector.map_ext.
    intros ?.
    rewrite encode_map_encode.
    cbn.
    rewrite (well_sized_inputs HWF i j k) by done.
    done.
  - cbn.
    rewrite vmap_bveval.
    apply bveval_ext.
    ...
    rewrite encode_map_encode.
    cbn.
    rewrite (well_sized_outputs HWF i j k) by done.
    done.
Qed.


#[universes(polymorphic=yes)]
Lemma partial_injection_extensionT `{Countable A, Infinite B}
  (l : list A) (f : A -> B) :
    ForallPairs (λ i j, f i = f j → i = j) l ->
    {g : A -> B | Inj (=) (=) g /\ Forall (fun a => g a = f a) l}.
Proof.
  intros Hlinj.
...
Lemma eval_sized_graph_ext {N T} {n m : btree N} {nf : N -> nat}
  (f f' : forall k, positive -> vec positive (nf k))
  (scohg : SizedCospanHyperGraph N T n m) :
  (forall k n, f k n = f' k n) ->
  eval_sized_graph nf f scohg = eval_sized_graph nf f' scohg.
Proof.
  intros Hf.
  apply cohg_ext.
  - cbn.
...
Section struct_isomorphic.
(* NB: This section is needed to fix some weird universe issues
  that otherwise occur between the following two lemmas *)

Context {N T : Type}.

(* #[universes(polymorphic=yes)] *)
Lemma eval_sized_graph_relabel_vecs_correct {n m : btree N}
  {nf : N -> nat} (idxs : forall k, positive -> vec positive (nf k))
  (f : N -> positive -> positive) {Hf : forall n, Inj eq eq (f n)}
  (scohg : SizedCospanHyperGraph N T n m) :
  well_sized scohg ->
  isomorphic
    (eval_sized_graph nf idxs scohg)
...
Proof.
  intros HWF.
  rewrite eval_sized_graph_relabel_vecs_gen by done.
  rewrite <- (reindex_graph_id (eval_sized_graph nf idxs scohg)) at 2.
  constructor; [|apply _].
  apply encode_map_inj.
  intros [k idx] [k' idx'].
  cbn.
  intros [= <- Heq].
  f_equal.
...
Lemma sized_graph_to_graph_struct_sized_isomorphic {n m}
...
Proof.
  ...
  specialize (eval_sized_graph_struct_sized_isomorphic (nf:=f) scohg scohg' HWF Heq).
  pose proof HWF as HWF'.
  rewrite Heq in HWF'.
  intros Heq'.
  etransitivity; [|etransitivity; [apply Heq'|]]; [|symmetry].
  - etransitivity.
    apply (subrel' isomorphic).
    refine ((fun Hinj => eval_sized_graph_relabel_vecs_correct _
      (λ k, ` (partial_injection_extensionT (fun_to_vec (λ i : fin (f k), Pos.of_succ_nat i))
        (λ i, default i ((bvec_to_vec (bvseq 1 (@nat_to_btree unit unit_inhabited (f k))) :> list _) !! (Nat.pred (Pos.to_nat i)))) (Hinj k)))
      ) _ scohg HWF).
    + intros n'.
      eapply proj1.
      match goal with
      |- context [` ?x] => apply (proj2_sig x)
      end.
    + intros k.
      specialize (NoDup_bvseq (nat_to_btree (A:=unit) (f k)) 1).
      remember (vec_to_list _) as l eqn:Hl.
      replace (vec_to_list _) with (Pos.of_succ_nat <$> seq 0 (length l)).
      2:{
        rewrite Hl, length_vec_to_list.
        rewrite vec_to_list_fun_to_vec.
        now rewrite bsize_nat_to_btree.
      }
      rewrite ForallPairs_map.
      rewrite ForallPairs_forall.
      clear Hl.
      intros Hl i j [_ Hi]%elem_of_seq [_ Hj]%elem_of_seq.
      replace (Nat.pred _) with i by lia.
      replace (Nat.pred _) with j by lia.
      cbn in Hi, Hj.
      apply lookup_lt_is_Some in Hi as [li Hli].
      apply lookup_lt_is_Some in Hj as [lj Hlj].
      rewrite Hli, Hlj.
      cbn.
      intros [= <-].
      now rewrite (NoDup_lookup _ i j _ Hl Hli Hlj).
    + cbn -[infinite_injection_avoiding].
      apply eq_reflexivity.
      apply eval_sized_graph_ext.
      intros k _.
      ...
      rewrite vlookup_cast.
      remember (Fin.cast _ _) as i' eqn:Hi'.
      replace (Nat.pred _) with (fin_to_nat i) by lia.
      replace (fin_to_nat i) with (fin_to_nat i') by (subst i'; now rewrite fin_to_nat_cast).
...
End struct_isomorphic.

(* Lemma eval_sized_graph_norm_sized_verts *)


Lemma scohg_to_cohg_map_vec_sized_graph_norm_sized_verts {N M} {T}
  {nf : N -> btree M} idxs
    (f : forall k, bvec M (nf k))
  {n m} (scohg : SizedCospanHyperGraph N T n m) :
  well_sized scohg ->
  scohg_to_cohg (map_vec_sized_graph idxs f (norm_sized_verts scohg)) =
  scohg_to_cohg (norm_sized_verts (map_vec_sized_graph idxs f scohg)).
Proof.
  intros HWF.
  apply cohg_ext; [|done..].
  apply hg_ext.
...
Lemma eval_sized_graph_norm_sized_verts {N T} {n m}
  (nf : N -> nat) (f : forall k, positive -> vec positive (nf k))
  (scohg : SizedCospanHyperGraph N T n m) :
  well_sized scohg ->
  eval_sized_graph nf f (norm_sized_verts scohg) =
  norm_verts (eval_sized_graph nf f scohg).
Proof.
  intros HWF.
  rewrite 2 (eval_sized_graph_to_map_vec_sized_graph (A:=unit)).
  rewrite scohg_to_cohg_map_vec_sized_graph_norm_sized_verts by done.
  rewrite scohg_to_cohg_norm_sized_verts.
  do 2 destruct (size_bind_nat_to_btree).
  now rewrite 2 cast_graph_id.
Qed.

Lemma sized_graph_to_graph_norm_sized_verts {N T} {n m}
  (f : N -> nat) (scohg : SizedCospanHyperGraph N T n m) :
  well_sized scohg ->
  sized_graph_to_graph f (norm_sized_verts scohg) = norm_verts (sized_graph_to_graph f scohg).
Proof.
  intros HWF.
  unfold sized_graph_to_graph.
  rewrite 2 (eval_sized_graph_to_map_vec_sized_graph (A:=unit)).
...
Lemma sized_graph_to_graph_cohg_syntactic_eq {N} `{Equiv T, Equivalence T equiv} {n m}
  ...
  well_sized scohg ->
  scohg ≡ₛ scohg' ->
  (sized_graph_to_graph f scohg ≡ₛ sized_graph_to_graph f scohg')%cohg.
Proof.
  intros HWF Heq.
  pose proof HWF as HWF'.
  rewrite Heq in HWF'.
  induction Heq.
  unfold sized_graph_to_graph.
  rewrite (eval_sized_graph_relabel_sized_graph _ _), eval_sized_graph_reindex_sized_graph.
  constructor; [apply _..|].
  rewrite (well_sized_relabel_sized_graph _), well_sized_reindex_sized_graph in HWF'.
  rewrite <- 2 eval_sized_graph_norm_sized_verts by done.
  now apply eval_sized_graph_scohg_eq.
Qed.
...
(* TODO: Think about changing definition of map_vec_sized_graph
  to add vertices to make sized_vertices_aux_map_vec_sized_graph
  unconditional *)
...
Lemma vertices_sized_graph_to_graph
  ...
  set_bind (λ p, list_to_set (default [] (
    n ← scohg.(sized_map) !! p;
    Some ((λ k, encode (p, Pos.of_succ_nat k)) <$> seq 0 (f n)))))
    (* Some (vec_to_list $ bvec_to_vec $ bvmap (λ k : positive, encode (p, k))
    (vec_to_nat_bvec (A:=unit) (fun_to_vec (λ i : fin (f n), Pos.of_succ_nat i))))))) *)
    (sized_vertices_aux scohg).
Proof.
  intros HWF.
  cbn.
  unfold sized_graph_to_graph.
  rewrite (eval_sized_graph_to_map_vec_sized_graph (A:=unit)).
  do 2 destruct size_bind_nat_to_btree.
...
(* Definition vec_bnorm_to_bvec {N A} {n : btree N} (v : bvec A (bsize (bnorm n))) *)

Lemma bsize_length {N} (n : btree N) : bsize n = length n.
Proof.
  induction n; [|done..].
  cbn.
  now rewrite length_app; f_equal.
Qed.

Lemma bsize_btree_of_list {N} (l : list N) : bsize (btree_of_list l) = length l.
Proof.
  induction l; cbn; f_equal; done.
Qed.

Lemma bsize_bnorm {N} {n : btree N} : bsize (bnorm n) = bsize n.
Proof.
  unfold bnorm.
  now rewrite bsize_btree_of_list, bsize_length.
Qed.

Definition bvmon_apply {N A} {n m : btree N} (p : bmonoidal n m) : bvec A n -> bvec A m :=
  match p with
  | bassoc => bvassoc
  | bassoci => λ v, (v.1 +++ v.2.1) +++ v.2.2
  | blunit => λ v, v.2
  | bluniti => λ v, bvnil +++ v
  | brunit => λ v, v.1
  | bruniti => λ v, v +++ bvnil
...
Fixpoint bvpath_apply {N A} {n m : btree N} (p : bpath n m) : bvec A n -> bvec A m :=
  match p with
  | brefl => λ v, v
  | bgen p => bvmon_apply p
  | bprop p q =>
    bvec_node_inv (λ _, bvec A (_ + _)) (λ v w, bvpath_apply p v +++ bvpath_apply q w)
  | btrans p q => bvpath_apply q ∘ bvpath_apply p
  end%bvec.

Lemma bsize_bpath {N} {n m : btree N} (p : bpath n m) : bsize n = bsize m.
Proof.
  rewrite 2 bsize_length.
  apply btree_elems_bpath in p.
  now f_equal.
Qed.

Lemma bvec_to_vec_bvmon_apply {N A} {n m : btree N} (p : bmonoidal n m) (v : bvec A n) : 
  bvec_to_vec (bvmon_apply p v) = 
  Vector.cast (bvec_to_vec v) (bsize_bpath p).
Proof.
  apply vec_to_list_inj2.
  rewrite vec_to_list_cast.
  cbn.
  induction p; cbn; inv_all_bvec;
  cbn; rewrite ?vec_to_list_app; cbn; 
  rewrite ?app_assoc, ?app_nil_r; done.
Qed.

Lemma bvec_to_vec_bvpath_apply {N A} {n m : btree N} (p : bpath n m) (v : bvec A n) : 
  bvec_to_vec (bvpath_apply p v) = 
  Vector.cast (bvec_to_vec v) (bsize_bpath p).
Proof.
  apply vec_to_list_inj2.
  rewrite vec_to_list_cast.
  ...
  - done.
  - cbn.
    now rewrite bvec_to_vec_bvmon_apply, vec_to_list_cast.
  - cbn.
    inv_all_bvec.
    cbn.
    rewrite 2 vec_to_list_app.
    f_equal; auto.
  - cbn.
    now rewrite IHp2, IHp1.
Qed.

Definition bvnorm {N A} {n : btree N} (v : bvec A n) : bvec A (bnorm n) :=
  bvpath_apply (bpath_to_norm n) v.

Definition sized_graph_paths_apply {N T} {n m n' m'}
  (p : bpath n n') (q : bpath m m') (scohg : SizedCospanHyperGraph N T n m) :
  SizedCospanHyperGraph N T n' m' :=
  mk_scohg (bvpath_apply p scohg.(sized_inputs))
    (bvpath_apply q scohg.(sized_outputs))
    scohg.(sized_hedges) scohg.(sized_map).

Lemma scohg_to_cohg_sized_graph_paths_apply {N T} {n m n' m'}
  (p : bpath n n') (q : bpath m m') (scohg : SizedCospanHyperGraph N T n m) :
  scohg_to_cohg (sized_graph_paths_apply p q scohg) =
  cast_graph (bsize_bpath p) (bsize_bpath q) (scohg_to_cohg scohg).
Proof.
  apply cohg_ext; [done|..].
  - apply bvec_to_vec_bvpath_apply.
  - apply bvec_to_vec_bvpath_apply.
Qed.

Lemma add_top_loops_cast_graph {T} {n n' m m'}
  (scohg : CospanHyperGraph T (n + m) (n + m'))
  (Hn : n + m = n' + m) (Hn' : n + m' = n' + m') :
  add_top_loops (cast_graph Hn Hn' scohg) = add_top_loops scohg.
Proof.
  assert (n = n') by lia.
  subst n'.
...
Lemma sized_add_top_loops_paths_apply {N T} {n n' m m'}
  (p : bpath n n') (scohg : SizedCospanHyperGraph N T (n + m) (n + m')) :
  sized_add_top_loops (sized_graph_paths_apply (bprop p brefl) (bprop p brefl) scohg) =
  sized_add_top_loops scohg.
Proof.
  apply scohg_ext'.
  - rewrite 2 scohg_to_cohg_sized_add_top_loops.
    rewrite scohg_to_cohg_sized_graph_paths_apply.
    apply add_top_loops_cast_graph.
  - rewrite 2 sized_map_sized_add_top_loops.
    destruct scohg as [ins outs hg sm]; inv_all_bvec.
    cbn.
    rewrite 2 bvec_to_vec_bvpath_apply.
    destruct (bsize_bpath _).
    now rewrite 2 cast_id.
Qed.

Definition bvbind_bpath {A B} (f g : A -> btree B)
  (Hfg : forall a, bpath (f a) (g a)) : forall (n : btree A), bpath (n ≫= f) (n ≫= g) :=
  fix go n :=
  match n with
  | n + m => bprop (go n) (go m)
  | !k => Hfg k
...
(* Lemma map_vec_sized_graph_to_by_vec
  {N} `{Inhabited M} {T} {n m : btree N} {nf : N -> btree M}
  (idxs : forall k, positive -> bvec positive (nf k))
  (f : forall k, bvec M (nf k)) (scohg : SizedCospanHyperGraph N T n m) :
  map_vec_sized_graph idxs f scohg = 
  (* map_vec_sized_graph idxs f idxs scohg. *)
  sized_graph_paths_apply (bvbind_bpath (λ n, nat_to_btree (bsize (nf n))) nf _ n)
    (bvbind_bpath (λ n, nat_to_btree (bsize (nf n))) nf _ m)
    (map_vec_sized_graph _ _ scohg).
map_vec_sized_graph *)
...
(* Lemma map_vec_sized_graph *)

Lemma bvec_to_vec_bvzip_with {N A B C} (f : A -> B -> C) {n : btree N}
  (v : bvec A n) (w : bvec B n) : 
  bvec_to_vec (bvzip_with f v w) = vzip_with f (bvec_to_vec v) (bvec_to_vec w).
Proof.
  induction n; inv_all_bvec; [|done..].
  cbn.
  rewrite vzip_with_app.
  now f_equal.
Qed.

From TensorRocq Require Hypergraphs.SizedGraph.Definitions.

Notation NatSizedCospanHyperGraph := Definitions.SizedCospanHyperGraph.

Definition scohg_to_nscohg {N T n m} (scohg : SizedCospanHyperGraph N T n m) :
  NatSizedCospanHyperGraph N T (bsize n) (bsize m) :=
  mk_scohg



Lemma map_vec_sized_graph_sized_add_top_loop {N M T} {k : N} {n m : btree N}
  {nf : N -> btree M} 
  (f : forall k, bvec M (nf k)) (scohg : SizedCospanHyperGraph N T (!k + n) (!k + m)) :
  map_vec_sized_graph (λ ) f (sized_add_top_loop scohg) =
  sized_add_top_loops (map_vec_sized_graph idxs f scohg).
Proof.
  apply scohg_ext'.
  - admit.
  - 
    destruct scohg as [ins outs hg sm].
    rewrite sized_map_sized_add_top_loops.
    cbn.
    inv_all_bvec.
    cbn -[map_relabel_one].
    rewrite <- bvec_to_vec_bvzip_with.
    rewrite bvzip_bvmap.
    bvec_to_vec bvzip_with
  ...
  - cbn.
    inv_all_bvec.
    cbn.
    rewrite bvbind_bvmap.


Lemma sized_graph_to_graph_add_top_loop {N T k n m}
  (f : N -> nat) (scohg : SizedCospanHyperGraph N T (!k + n) (!k + m)) :
  well_sized scohg ->
  (sized_graph_to_graph f (sized_add_top_loop scohg) ≡ᵥ
  add_top_loops (sized_graph_to_graph f scohg))%cohg.
Proof.
  intros HWF.
